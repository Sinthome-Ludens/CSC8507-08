    ...
    float panelX, float panelY, float panelW, float panelH)
{
    ...
    ImU32 gridColor = IM_COL32(200, 208, 218, 30);
    ...
    // Radar rings (black accent)
    draw->AddCircle(ImVec2(cx, cy), maxR,        IM_COL32(0, 0, 0, 30), 64, 1.0f);
    draw->AddCircle(ImVec2(cx, cy), maxR * 0.6f, IM_COL32(0, 0, 0, 25), 48, 1.0f);
    draw->AddCircle(ImVec2(cx, cy), maxR * 0.3f, IM_COL32(0, 0, 0, 20), 32, 1.0f);
    ...
    for (int i = 0; i < nodeCount; ++i) {
        ...
        draw->AddCircleFilled(ImVec2(nx, ny), nodeR,
            IM_COL32(0, 0, 0, nodeAlpha));

        if (pulse > 0.6f) {
            draw->AddLine(ImVec2(nx, ny), ImVec2(cx, cy),
                IM_COL32(0, 0, 0, (uint8_t)(std::min(20.0f * pulse, 255.0f))), 1.0f);
        }
    }
...
void RenderSplashScreen(Registry& registry, float dt) {
    ...
    draw->AddText(ImVec2(titleX, titleY), IM_COL32(0, 0, 0, 60), title);
    draw->AddText(ImVec2(titleX - 1, titleY), IM_COL32(0, 0, 0, 100), title);
    draw->AddText(ImVec2(titleX, titleY), IM_COL32(0, 0, 0, 255), title);
    ...
    draw->AddText(ImVec2(subX, subY), IM_COL32(0, 0, 0, 180), subtitle);
    ...
    draw->AddText(ImVec2(promptX, promptY),
        IM_COL32(0, 0, 0, promptAlpha), prompt);
    ...
    draw->AddText(ImVec2(teamX, teamY), IM_COL32(0, 0, 0, 160), team);
    ...
    if (ui.splashTimer > 0.5f) {
        ...
        if (kb) {
            for (int k = 0; k < (int)KeyCodes::MAXVALUE; ++k) {
                if (kb->KeyPressed(static_cast<KeyCodes::Type>(k)) && k >= (int)KeyCodes::BACK) {
                    anyInput = true;
                    break;
                }
            }
...
void RenderMainMenu(Registry& registry, float /*dt*/) {
    ...
    draw->AddText(ImVec2(vpPos.x + panelPadX, subtitleY),
        IM_COL32(0, 0, 0, 150), "TACTICAL INFILTRATION v1.0");
    ...
    for (int i = 0; i < kMenuItemCount; ++i) {
        ...
        // Selected item highlight (black accent)
        if (isSelected) {
            draw->AddRectFilled(itemMin, itemMax,
                IM_COL32(0, 0, 0, 15), 2.0f);
            draw->AddRect(itemMin, itemMax,
                IM_COL32(0, 0, 0, 60), 2.0f, 0, 1.0f);
        }
        ...
        ImU32 textColor = isSelected ? IM_COL32(0, 0, 0, 255)
                                     : IM_COL32(0, 0, 0, 220);
...
void RenderSettingsScreen(Registry& registry, float /*dt*/) {
    ...
    draw->AddRectFilled(
        ...
        IM_COL32(246, 247, 250, 245), 3.0f);
    draw->AddRect(
        ImVec2(panelX, panelY),
        ImVec2(panelX + panelW, panelY + panelH),
        IM_COL32(215, 220, 228, 120), 3.0f, 0, 1.0f);
    ...
    ImFont* titleFont = UITheme::GetFont_TerminalLarge();
    if (titleFont) ImGui::PushFont(titleFont);
    draw->AddText(ImVec2(contentX, contentY), IM_COL32(0, 0, 0, 255), "SETTINGS");
    if (titleFont) ImGui::PopFont();
    ...
    // Display section
    ImGui::TextColored(ImVec4(0.00f, 0.00f, 0.00f, 1.0f), "DISPLAY");
    ImGui::Spacing();
    ...
    // Audio section
    ImGui::TextColored(ImVec4(0.00f, 0.00f, 0.00f, 1.0f), "AUDIO");
    ImGui::Spacing();

    ImGui::Text("Master Volume:");
    ImGui::SameLine(160.0f);
    ImGui::PushStyleColor(ImGuiCol_SliderGrab, ImVec4(0.00f, 0.00f, 0.00f, 1.0f));
    ImGui::PushStyleColor(ImGuiCol_SliderGrabActive, ImVec4(0.20f, 0.20f, 0.20f, 1.0f));
    {
    ...
    // Controls section
    ImGui::TextColored(ImVec4(0.00f, 0.00f, 0.00f, 1.0f), "CONTROLS");
    ImGui::Spacing();
    ...
    if (ImGui::Button("< BACK", ImVec2(120, 35))) {
        ...
        LOG_INFO("[UI_Menus] Settings -> " << (int)backTo);
    }
...
void RenderPauseMenu(Registry& registry, float /*dt*/) {
    ...
    draw->AddText(ImVec2(titleX, panelY + 25.0f), IM_COL32(0, 0, 0, 255), title);
    ...
    for (int i = 0; i < kPauseItemCount; ++i) {
        ...
        if (isSelected) {
            draw->AddRectFilled(itemMin, itemMax, IM_COL32(0, 0, 0, 15), 2.0f);
            draw->AddRect(itemMin, itemMax, IM_COL32(0, 0, 0, 60), 2.0f, 0, 1.0f);
        }
