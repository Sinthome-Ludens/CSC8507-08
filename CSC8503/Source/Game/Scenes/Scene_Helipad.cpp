 ...
 * @brief Helipad 关卡场景生命周期实现（资源加载、实体生成、系统注册）。
 */
                            ...
                            const Res_NCL_Pointers& /*nclPtrs*/)
{
    ...
    // 碰撞体：优先加载 _collision.obj，不存在则 fallback 到渲染用 .obj
    {
        ...
        if (!mapCollLoaded || mapCollVerts.empty()) {
            collObjPath = NCL::Assets::MESHDIR + "Helipad.obj";
            mapCollLoaded = ECS::AssimpLoader::LoadCollisionGeometry(
                collObjPath, mapCollVerts, mapCollIndices);
            LOG_WARN("[Scene_Helipad] _collision.obj not found, falling back to Helipad.obj");
        }
        ...
        if (mapCollLoaded && !mapCollVerts.empty()) {
            ...
            LOG_INFO("[Scene_Helipad] Collision mesh loaded from " << collObjPath
                     << " (" << mapCollVerts.size() << " verts, " << mapCollIndices.size() / 3 << " tris)");
        } else {
            LOG_WARN("[Scene_Helipad] Failed to load any collision geometry!");
        }
    }
    ...
    m_Pathfinder->LoadNavMesh(NCL::Assets::MESHDIR + "Helipad.navmesh");
    ...
    // ── 终点区域生成 ────────────────────────────────────────────────────
    {
...

        {
            ...
            LOG_INFO("[Scene_Helipad] Finish zone: render at map origin, "
    ...
    {
        auto points = ECS::LoadMapPoints(NCL::Assets::MESHDIR + "Helipad.startpoints");
        if (!points.loaded) {
            points = ECS::LoadMapPoints(NCL::Assets::MESHDIR + "Helipad.points");
        }
    ...
    {
        auto enemyData = ECS::LoadEnemySpawns(NCL::Assets::MESHDIR + "Helipad.enemyspawns");
        if (enemyData.loaded) {
            ...
            LOG_INFO("[Scene_Helipad] Spawned " << enemyData.spawns.size()
    ...
    if (registry.has_ctx<ECS::Res_UIState>()
     ...
     && registry.has_ctx<ECS::Res_ItemInventory2>()) {
        ...
        LOG_INFO("[Scene_Helipad] Equipment synced from MissionSelect: items=["
