/**
 * @file Scene_Dock.cpp
 * @brief Dock 关卡场景生命周期实现（资源加载、实体生成、系统注册）。
 */
                         ...
                         const Res_NCL_Pointers& /*nclPtrs*/)
{
    ...
    {
        std::string collObjPath = NCL::Assets::MESHDIR + "Dock_collision.obj";
        ...
        if (!mapCollLoaded || mapCollVerts.empty()) {
            collObjPath = NCL::Assets::MESHDIR + "Dock.obj";
            mapCollLoaded = ECS::AssimpLoader::LoadCollisionGeometry(
                collObjPath, mapCollVerts, mapCollIndices);
            LOG_WARN("[Scene_Dock] _collision.obj not found, falling back to Dock.obj");
        }
        ...
        if (mapCollLoaded && !mapCollVerts.empty()) {
            ...
            LOG_INFO("[Scene_Dock] Collision mesh loaded from " << collObjPath
                     << " (" << mapCollVerts.size() << " verts, " << mapCollIndices.size() / 3 << " tris)");
        } else {
            LOG_WARN("[Scene_Dock] Failed to load any collision geometry!");
        }
    }
    ...
    m_Pathfinder->LoadNavMesh(NCL::Assets::MESHDIR + "Dock.navmesh");
    ...
    // ── 终点区域生成 ────────────────────────────────────────────────────
    {
...

        {
            ...
            LOG_INFO("[Scene_Dock] Finish zone: render at map origin, "
    ...
    {
        auto points = ECS::LoadMapPoints(NCL::Assets::MESHDIR + "Dock.startpoints");
        if (!points.loaded) {
            points = ECS::LoadMapPoints(NCL::Assets::MESHDIR + "Dock.points");
        }
    ...
    {
        auto enemyData = ECS::LoadEnemySpawns(NCL::Assets::MESHDIR + "Dock.enemyspawns");
        if (enemyData.loaded) {
            ...
            LOG_INFO("[Scene_Dock] Spawned " << enemyData.spawns.size()
    ...
    if (registry.has_ctx<ECS::Res_UIState>()
     ...
     && registry.has_ctx<ECS::Res_ItemInventory2>()) {
        ...
        LOG_INFO("[Scene_Dock] Equipment synced from MissionSelect: items=["
