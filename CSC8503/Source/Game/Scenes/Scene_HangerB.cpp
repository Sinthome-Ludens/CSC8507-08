                            ...
                            const Res_NCL_Pointers& /*nclPtrs*/)
{
    ...
    ECS::MeshHandle mapMesh = ECS::AssetManager::Instance().LoadMesh(
        NCL::Assets::MESHDIR + "HangerB_collision.obj");
    ...
    {
        std::string collObjPath = NCL::Assets::MESHDIR + "HangerB_collision.obj";
        ...
        if (!mapCollLoaded || mapCollVerts.empty()) {
            collObjPath = NCL::Assets::MESHDIR + "HangerB.obj";
            mapCollLoaded = ECS::AssimpLoader::LoadCollisionGeometry(
                collObjPath, mapCollVerts, mapCollIndices);
            LOG_WARN("[Scene_HangerB] _collision.obj not found, falling back to HangerB.obj");
        }
        ...
        if (mapCollLoaded && !mapCollVerts.empty()) {
            ...
            LOG_WARN("[Scene_HangerB] Failed to load any collision geometry!");
        }
    }
    ...
    // ── 终点区域生成 ────────────────────────────────────────────────────
    {
...

        {
            ...
            LOG_INFO("[Scene_HangerB] Finish zone: render at map origin, "
    ...
    {
        auto points = ECS::LoadMapPoints(NCL::Assets::MESHDIR + "HangerB.startpoints");
        if (!points.loaded) {
            points = ECS::LoadMapPoints(NCL::Assets::MESHDIR + "HangerB.points");
        }
    ...
    {
        auto enemyData = ECS::LoadEnemySpawns(NCL::Assets::MESHDIR + "HangerB.enemyspawns");
        if (enemyData.loaded) {
            ...
            LOG_INFO("[Scene_HangerB] Spawned " << enemyData.spawns.size()
    ...
    if (registry.has_ctx<ECS::Res_UIState>()
     ...
     && registry.has_ctx<ECS::Res_ItemInventory2>()) {
        ...
        LOG_INFO("[Scene_HangerB] Equipment synced from MissionSelect: items=["
