                            ...
                            const Res_NCL_Pointers& /*nclPtrs*/)
{
    ...
    ECS::MeshHandle mapMesh = ECS::AssetManager::Instance().LoadMesh(
        NCL::Assets::MESHDIR + "HangerA_collision.obj");
    ...
    {
        std::string collObjPath = NCL::Assets::MESHDIR + "HangerA_collision.obj";
        ...
        if (!mapCollLoaded || mapCollVerts.empty()) {
            collObjPath = NCL::Assets::MESHDIR + "HangerA.obj";
            mapCollLoaded = ECS::AssimpLoader::LoadCollisionGeometry(
                collObjPath, mapCollVerts, mapCollIndices);
            LOG_WARN("[Scene_HangerA] _collision.obj not found, falling back to HangerA.obj");
        }
        ...
        if (mapCollLoaded && !mapCollVerts.empty()) {
            ...
            LOG_WARN("[Scene_HangerA] Failed to load any collision geometry!");
        }
    }
    ...
    // 同一实体挂 C_T_FinishZone，Sys_LevelGoal 用 OBJ 几何中心做距离检测
    {
        ...
        // 2. 检测实体（不可见，放在 OBJ 几何中心的世界坐标）
        {
            ...
            LOG_INFO("[Scene_HangerA] Finish zone: render at map origin, "
    ...
    {
        auto points = ECS::LoadMapPoints(NCL::Assets::MESHDIR + "HangerA.startpoints");
        if (!points.loaded) {
            points = ECS::LoadMapPoints(NCL::Assets::MESHDIR + "HangerA.points");
        }
    ...
    {
        auto enemyData = ECS::LoadEnemySpawns(NCL::Assets::MESHDIR + "HangerA.enemyspawns");
        if (enemyData.loaded) {
            ...
            LOG_INFO("[Scene_HangerA] Spawned " << enemyData.spawns.size()
